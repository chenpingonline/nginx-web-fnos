...
[返回首页](../README.md) · [分步使用指南](user-guide.md)
...
- 规则表格展示名称、完整访问入口、转发目标、配置状态、时段平均请求速率和错误率；支持搜索、协议与配置状态筛选、请求数/错误数排序和分页。名称打开该规则的响应趋势，错误率箭头打开该规则的错误详情。
...
- 在同一页面按分组卡片展示规则，支持展开和收起，顶部提供分组筛选；分组可设置默认监听类型、协议、监听端口、证书和 HTTP/2。新规则默认继承，各项可取消继承并自定义。
- 已有规则移动分组时保留配置；修改分组只更新继承项，删除分组会保留规则当前配置并移至未分组。分组保存后应用全部草稿，失败时保留草稿并提示。
- 按分组、HTTP/HTTPS 协议、启用状态与名称/域名/端口/目标组合筛选，显示匹配数量并支持一键重置。
...
- 分组及单条规则支持 IPv4、IPv6 或双栈监听，规则可继承分组的监听类型。
- 历史配置未指定类型时保持 IPv4；为已有规则启用 IPv6 时，可直接编辑规则，或勾选继承分组的“监听类型”。
- IPv6 入口可转发到 IPv4 内网服务；IPv4 监听地址为 `0.0.0.0:端口`，IPv6 为 `[::]:端口`。双栈需要系统支持 IPv6，且对应防火墙端口已放行。
...
- 支持 RSA 2048/4096、ECDSA P-256/P-384，以及续期时轮换私钥。申请前需同意所选 CA 服务条款。
...
- 证书列表“修改”可在弹窗编辑已关联 ACME 任务的签发域名并重新签发，复用原 DNS 凭据和账户；签发成功后沿用证书 ID 自动替换，失败保留旧证书。导入且未关联 ACME 的证书需要先配置申请信息。

## 运行日志

- 查看最近的 Nginx 错误日志、HTTP 访问日志、Stream 访问日志和管理服务日志。
- 默认读取最近 500 行，可选择读取行数；搜索只针对当前已加载日志，搜索时暂停自动刷新，清空后恢复。
- 有搜索内容时显示匹配数量及上一个/下一个按钮。默认不自动换行，通过底部横向滚动条查看长行；可勾选“自动换行”。
...
- 可放弃当前全部未应用修改，恢复为 Nginx 正在使用的配置；此操作不会重载 Nginx，也不会生成新的历史版本。
- 删除不再需要的历史记录，并配置最多保留 1～100 个版本。

## Nginx 配置

- 只读查看当前实际使用的 `nginx.conf` 和生成的 HTTP/Stream 配置片段。
- 主配置单独展示，其余文件按系统配置、HTTP(S) 代理、TCP/UDP 代理分类下拉选择，显示当前文件名并支持复制。
- 系统生成文件保持只读；可将现有文件另存为自定义配置，也可新建、编辑和删除 `custom-*.conf`（HTTP）或 `custom-*.stream`（TCP/UDP Stream）文件。
- 自定义配置进入草稿、配置历史和备份。应用前会与系统生成配置一起执行 `nginx -t`；校验或激活失败时恢复原运行配置。
- 自定义配置允许原始 Nginx 指令，不受结构化表单的安全限制，仅应由理解 Nginx 配置的管理员使用。
...
- 同版本提供标准版（监听 1024–65535）与全端口版（监听 1–65535，含 80/443）；标准版使用应用用户权限，全端口版生命周期脚本短暂使用 root 为内置 Nginx 设置 `CAP_NET_BIND_SERVICE` 后降权，管理服务和 Nginx 均以普通 `nginx-web` package 用户运行。
